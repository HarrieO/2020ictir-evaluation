...
import policies.optimizedinterleaving as OI
...
print('Starting Optimization for Interleaving')
policy = OI.OptimizedInterleaving(
                data.train,
                models_train_rankings,
                models_train_inv_rankings,
                cutoff,
              )
...
results = {
  'model name': 'Optimized Interleaving',
  'model type': 'Optimized Interleaving',
...
for sample_i in range(n_queries_to_sample):
  ...
  # qid = np.random.choice(1000)
  q_n_docs = data.train.query_size(qid)
  ranking_len = np.minimum(q_n_docs, cutoff)
  s_i, e_i = data.train.query_range(qid)

  interleaving, scores = policy.sample(qid)
  ...
  if np.any(clicks):
    ...
    estimates[sample_i] = np.sum(scores[interleaving[clicks]])
