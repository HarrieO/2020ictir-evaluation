...
def sample_from_click_probs(click_probs):
  return np.where(bernoilli_sample_from_probs(click_probs))[0]

def bernoilli_sample_from_probs(probs):
  coin_flips = np.random.uniform(size=probs.shape)
  return np.less(coin_flips, probs)

def inverse_rank_prob(inv_ranking, eta):
    return (1./(inv_ranking+1.))**eta

def get_relevance_click_model(click_model):
  if 'binarized' in click_model:
    max_prob = float(click_model.replace('binarized', ''))
    def relevance_click_prob(labels):
      n_docs = labels.shape[0]
      rel_prob = np.full(n_docs, 0.1)
      rel_prob[labels>2] = max_prob
      return rel_prob
    ...
    def relevance_click_prob(labels):
      ...
      return rel_prob
  elif 'alpha' in click_model:
    click_model = click_model.replace('alpha', '')
    click_model = click_model.replace('beta', ' ')
    alpha, beta = click_model.split()
    min_prob = float(alpha)
    lin_scalar = float(beta)
    def relevance_click_prob(labels):
      n_docs = labels.shape[0]
      rel_prob = np.full(n_docs, min_prob)
      rel_prob += labels*lin_scalar
      return rel_prob
  elif 'quadratic' in click_model:
    click_model = click_model.replace('quadratic', '')
    max_prob = float(click_model)
    min_prob = float(0.1)
    def relevance_click_prob(labels):
      n_docs = labels.shape[0]
      rel_prob = np.full(n_docs, min_prob)
      rel_prob += labels**2./16.*(max_prob-min_prob)
      return rel_prob
  else:
