...
parser.add_argument("--tau", type=float,
                    default=4.0,
                    help="Temperature for ranking distribution.")
...
results = {
  'model name': 'Probabilistic Interleaving',
  'model type': 'Probabilistic Interleaving',
...
train_prob_scores = 1./(models_train_inv_rankings+1.)**tau
...
for sample_i in range(n_queries_to_sample):
  ...
  ind = np.arange(q_n_docs)
  interleaving = np.zeros(ranking_len, dtype=np.int32)
  add_prob = np.zeros((2, ranking_len), dtype=np.float64)
  nom = train_prob_scores[:, s_i:e_i].copy()
  for i in range(ranking_len):
    denom = np.sum(nom, axis=1)
    model_prob = nom/denom[:, None]
    doc_prob = np.mean(model_prob, axis=0)
    sampled_doc = np.random.choice(ind, p=doc_prob)

    add_prob[:, i] = model_prob[:, sampled_doc]
    interleaving[i] = sampled_doc
    nom[:, sampled_doc] = 0.

  add_prob /= np.sum(add_prob, axis=0)[None, :]
  ...
  if np.any(clicks):
    ...
    if n_clicks == 1:
      o_prob = add_prob.T[clicks][0, :]
      estimates[sample_i] = o_prob[0] - o_prob[1]
    else:
      o_prob = add_prob.T[clicks].T
      comb = np.array(list((itertools.product([0, 1], repeat=n_clicks))))
      not_tie = np.sum(comb, axis=1) != n_clicks*0.5
      comb = comb[not_tie]
      wins = np.sum(comb, axis=1) < n_clicks*0.5
      o_i = np.arange(o_prob.shape[1])[None, :]
      prob_per_comb = np.prod(o_prob[comb, o_i], axis=1)
      
      estimates[sample_i] = 2.*np.sum(prob_per_comb[wins]) - np.sum(prob_per_comb)
